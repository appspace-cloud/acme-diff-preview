...
```bash
python3 -m pytest tests/ -q                                  # full suite (500 tests)
python3 -m pytest tests/ -q --cov=src --cov-report=term      # with the coverage report
```

**Coverage: 96.9% of `src/` overall — `dev_hard_refresh.py` at 99%, `diff_preview.py` at 97%** — measured over five complementary layers, all of which run with **zero external infrastructure** (no cluster, no Bitbucket account, no registry, no real helm), on any machine and in CI:
...
The remaining ~3% is genuinely race-dependent interleavings (a thread losing a rename race mid-pull), OS-level failure injections (rename returning EBUSY), and defensive `except` arms around them — branches whose tests would have to orchestrate the failure so artificially that they would validate the test scaffolding, not the service.
