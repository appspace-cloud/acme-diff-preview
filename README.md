...
```bash
python3 -m pytest tests/ -q                                  # full suite (523 tests)
python3 -m pytest tests/ -q --cov=src --cov-report=term      # with the coverage report
```

**Coverage: 98.5% of `src/` overall — `dev_hard_refresh.py` at 99%, `diff_preview.py` at 98%** — measured over six complementary layers, all of which run with **zero external infrastructure** (no cluster, no Bitbucket account, no registry, no real helm), on any machine and in CI:
...
The remaining ~1.5% (37 lines) is genuinely out of honest reach: OS-level failure injections (rename returning EBUSY mid-park), hardcoded 30–60 second waits (the singleflight timeout, the hard-refresh subprocess timeout), thread interleavings that cannot be forced deterministically, and the defensive post-loop raises behind them — branches whose tests would validate the test scaffolding, not the service.
