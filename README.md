...
```bash
python3 -m pytest tests/ -q                                  # full suite (416 tests)
python3 -m pytest tests/ -q --cov=src --cov-report=term      # with the coverage report
```

**Coverage: 86.6% of `src/` overall — `dev_hard_refresh.py` at 97%, `diff_preview.py` at 86%** — measured over three complementary layers, all of which run with **zero external infrastructure** (no cluster, no Bitbucket account, no registry), on any machine and in CI:
