...
```bash
python3 -m pytest tests/ -q                                  # full suite (440 tests)
python3 -m pytest tests/ -q --cov=src --cov-report=term      # with the coverage report
```

**Coverage: 90.4% of `src/` overall — `dev_hard_refresh.py` at 97%, `diff_preview.py` at 90%** — measured over four complementary layers, all of which run with **zero external infrastructure** (no cluster, no Bitbucket account, no registry, no real helm), on any machine and in CI:
...
The remaining ~10% is the deepest error branches (concurrent-race fallbacks, folder-move rename chains inside `_run_one_diff`, the new-environment render's structural-classification tail) where each additional test buys a line or two of highly conditional code — past the point where more coverage means more confidence.
