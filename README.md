...
```bash
python3 -m pytest tests/ -q                                  # full suite (711 tests)
python3 -m pytest tests/ -q --cov=src --cov-report=term      # with the coverage report
```

**Coverage: 100% of `src/`** — measured over eight complementary layers, all of which run with **zero external infrastructure** (no cluster, no Bitbucket account, no registry, no real helm), on any machine and in CI. A handful of lines are excluded as genuinely unreachable defensive guards (documented inline with `# pragma: no cover` and a one-line reason each — e.g. a post-retry-loop fallback that can never be reached because every loop iteration already returns or raises, or a Python-version-specific branch modern `email.utils.parsedate_to_datetime` no longer takes) plus the standard `if __name__ == "__main__":` entry guards.
...
- **Coverage-to-100% pass** (v2.6.4, `test_v264_coverage_*.py`) — closed the last 47 lines a fresh per-line gap analysis found after the precision sweep: the `_pooled_urlopen` reused-socket rearm and cleanup-close-failure paths, a real still-queued future actually cancelled (not just asserted) when a render times out, the `_oci_selfcheck` login-failure branch, `find_existing_comment`'s fast-path raise contract with the correct multi-repo cache key, every branch of the input-changes panel (`_summarize_input_changes`: non-YAML skip, unparseable YAML, no-scannable-keys, per-file and cross-file budget truncation), `main_iteration` self-healing a corrupted `_main_render_sha` and evicting a legacy non-tuple `_seen` key, and a dozen pure-helper edges (retry-after date parsing, DIFF_REPOS/app-git-repo parsing skips, `NO_PROXY=*`, replicas-zeroed value guards, the precomputed-facts overflow note). Six lines turned out to be genuinely unreachable defensive guards once traced through fully (a post-retry-loop fallback no iteration can fall through to, a stale-connection close that can never find a connection still there, and a Python-version-specific `None` check `parsedate_to_datetime` no longer produces) — each marked `# pragma: no cover` inline with a one-line reason rather than contorted into a test that would prove nothing. Result: 100% of `src/`, 711 tests.
